...
Ball.prototype.intersect = function( ray, existing_intersection, minimum_dist )
{
  ...
  // there is intersect, need to update
  existing_intersection.ball = this;
  if( t < existing_intersection.distance ) existing_intersection.distance = t; // update if closer
  ...
  var normal = inside ? subtract( intersection_point, this.position.concat(1) ) : subtract( this.position.concat(1), intersection_point ); // for unit sphere
  var tranposed_inverse = transpose( this.inverse_transform );
  existing_intersection.normal = normalize( mult_vec( tranposed_inverse, normal ) ); // for transformed sphere
...
Raytracer.prototype.trace = function( ray, color_remaining, shadow_test_light_source )
{
  ...
  if( closest_intersection.ball && shadow_test_light_source ) return true;
  if( !closest_intersection.ball && shadow_test_light_source ) return false;

  // Phong Illumination Model
  var color_local = vec3();

  for( var i = 0; i < this.anim.graphicsState.lights.length; i++ ){
    var light_ray = { origin: closest_intersection.intersect_point, dir: subtract( this.anim.graphicsState.lights[i].position, closest_intersection.intersect_point ) };
    if( !this.trace( light_ray, color_remaining, true ) ){

    var l = normalize( subtract( this.anim.graphicsState.lights[i].position, closest_intersection.intersect_point ) );
    var v = normalize( subtract( ray.origin, closest_intersection.intersect_point ) );
    // r = 2n(n.l) - l
    var r = normalize( subtract( scale_vec( 2 * dot(closest_intersection.normal, l), closest_intersection.normal ), l ) );

    var color_ambient = scale_vec( closest_intersection.ball.k_a, closest_intersection.ball.color );
    var color_diffuse = mult( this.anim.graphicsState.lights[i].color.slice(0, 3), scale_vec( closest_intersection.ball.k_d * Math.max( dot(closest_intersection.normal, l), 0 ), closest_intersection.ball.color ) );
    var color_specular = scale_vec( closest_intersection.ball.k_s, scale_vec( Math.pow( dot(r, v), closest_intersection.ball.n ), vec3(1, 1, 1) ) );
    color_local = add( color_local, add( color_ambient, add( color_diffuse, color_specular ) ) );
    }
    else continue;
  }
  ...
  // reflection
    // v = -2(n.c)n + c
  var ray_reflect_dir = add( scale_vec( -2 * dot( closest_intersection.normal, ray.dir ), closest_intersection.normal ), ray.dir );
  var ray_reflect = { origin: closest_intersection.intersect_point, dir: ray_reflect_dir };
  ...
  var color_reflect = scale_vec( closest_intersection.ball.k_r, this.trace( ray_reflect, color_remaining_reflect ).slice(0, 3) );


  // refraction
  // var ray_refract_ori = 
  // var ray_refract_dir = 
  // var ray_refract = { origin: ray_refract_ori, dir: ray_refract_dir };
  ...
  // var color_refract = trace( ray_refract, color_remaining_refract );

  //var reflect_and_refract = add( (closest_intersection.ball.k_r * color_reflect.slice(0, 3)), (closest_intersection.ball.k_refract * color_refract.slice(0, 3)) );
  var pixel_color = add( color_local, mult( subtract( vec3(0, 0, 0) , color_local ), color_reflect ) );
