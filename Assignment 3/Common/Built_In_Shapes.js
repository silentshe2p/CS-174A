  ...
  { 
    ...
    for( var i = 0; i <= rows; i++ )        // Travel down the curve spelled out by the parameter "points"
    {
      ...
      for( var j = 0; j <= columns; j++ )
      {
        ...
        recipient.positions.push( mult_vec( spin, currPoint ).slice(0,3) );            if( !flat_shade ) recipient.normals.push( mult_vec( spin, normal ).slice(0,3) );
        recipient.texture_coords.push( vec2( j, - i ) );
      }
    }
    ...
    for( var i = offset; i < recipient.positions.length; i++ )                         // Apply points_transform to all points added during this call
      { recipient.positions[i] = mult_vec( points_transform, recipient.positions[ i ].concat( 1 ) ).slice(0,3);    
        recipient.normals[i]   = mult_vec( transpose( inverse( points_transform ) ), recipient.normals[ i ].concat( 1 ) ).slice(0,3);     }
...
Make_Shape_Subclass( "N_Polygon", Triangle_Fan );
  N_Polygon.prototype.populate = function( recipient, n, points_transform = mat4() )  
    {  
      ...
      for( var i = 0; i < n; i++ )     this.add_fan_point( recipient, offset, vec3( Math.sqrt(2) * Math.cos( Math.PI / 4 + i/n * 2*Math.PI ), Math.sqrt(2) * Math.sin( Math.PI / 4 + i/n * 2*Math.PI ), 0 ), vec3(0, 0, 1), vec2( .5 + .5 * Math.sqrt(2) * Math.cos( Math.PI / 4 + i/n * 2*Math.PI ), .5 + .5 * Math.sqrt(2) * Math.sin( Math.PI / 4 + i/n * 2*Math.PI ) ) ); 
      ...
      for( var i = offset; i < recipient.positions.length; i++ )          
        {
          ...
          recipient.positions[i] = mult_vec( points_transform, recipient.positions[i].concat( 1 ) ).slice(0,3);       // Apply points_transform to all points added during this call.
        }
